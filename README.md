...
当前结果（`data/eval/results/2026-09-22_prompt-v3b-orig.json`）：

| 指标 | 结果 |
|---|---|
| 回答类型准确率 | 0.99（75 轮错 1 轮：问“R3 等级代表什么”时引用揭示书说明库里只有 2 级，没有走固定拒答） |
| 负例拒答率 / 正例误拒率 | 0.92 / 0 |
| 合规违规（禁用表达、投资建议措辞） | 0 |
| 必含内容通过率 | 0.96 |
| 多轮对话 | 4 / 4 |
| 证据中包含标准原句 / 引用的文件包含标准文件 | 0.98 / 1.0 |
| 单次问答耗时：中位数 / p90 | 6.4 s / 12.6 s |

评测集从建立起一直用于调参，为检验是否过拟合，另建了留出集 `data/eval/fin_eval_holdout.jsonl`（73 题、81 轮，题目与标准原句全部新出，不参与调参）。留出集最新一轮（`2026-09-22_prompt-v3b-holdout.json`）：回答类型准确率 0.99，负例拒答率 1.0，合规违规 0，必含内容 0.97，多轮对话 3 / 4。
...
- 回答类型由模型判断，温度设为 0 也会有波动，个别题的归类可能因此变化；偶尔元信息与正文不一致（正文是澄清话术却标成回答，或正文引用了证据却标成拒答）。
