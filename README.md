...
node_query_plan → node_gather_evidence → node_answer_output
  改写问题、抽对象    对象解析 + 财务事实 / 文档摘要 / 语义检索 + 精排 + 编号    一次调用定判定与正文，再过闸门
```
...
- **按对象取证据**：上市公司数值题查结构化财务事实（季报表格确定性解析），有对象时附文档摘要并把检索限定在其文档内，其余走稠密 + 稀疏混合检索 + 云端精排。
...
规则打分（不用 LLM 裁判），67 题 75 个轮次，存档 `data/eval/results/2026-09-20_prompt-first-v2.json`：

| 指标 | 提示词主导 | 上一版（代码路由） |
|---|---|---|
| 行为准确率（回答 / 拒答 / 澄清 / 不提供建议 / 时效提示是否符合预期） | 0.973 | 0.987 |
| 负例拒答率 / 正例误拒率 | 1.0 / 0.018 | 0.923 / 0 |
| 合规违规（禁用表达、投资建议措辞） | 0 | 0 |
| 必含内容通过率（三份季报与统计公报的数值题全部原值命中） | 0.96 | 0.96 |
| 多轮链（含澄清流程） | 3 / 4 | 4 / 4 |
| 证据含金标原句 / 引用文件含金标文件 | 0.927 / 1.0 | 0.964 / 1.0 |
| 延迟中位 / p90 | 3.8 s / 5.8 s | 5.7 s / 10.4 s |

该轮之后又按两处失败调整了提示词（“适合哪类投资者”不算投资建议、证据措辞不一致时不得判未披露），尚未重跑评测。
...
- 判定交给模型后，个别题的归类会随模型状态波动（最近一轮：“适合稳健型用户吗”被判为不提供建议、“赎回多久到账”被判为资料未披露，已按此调整提示词但未重跑评测）。
- CPU 上 BGE-M3 编码约 1.7 秒/切片，全量导入 17 个文件约 10 分钟；问答中位延迟约 4 秒。
