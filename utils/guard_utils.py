...
def violations(sentence: str, context: str = "") -> list[str]:
    """context：用于判断“描述骗局”语境的额外文本（用户问题、已输出的内容）。"""
    hits: list[str] = []
...
class StreamGuard:
    ...
    def __init__(self, context: str = "") -> None:
        self.buf = ""
        self.context = context  # 用户问题；已放行的句子会陆续追加进来
        self.hits: list[str] = []
    ...
    def feed(self, delta: str) -> str:
        ...
        out: list[str] = []
        while True:
            idx = min((i for i in (self.buf.find(c) for c in SENTENCE_END) if i >= 0), default=-1)
            if idx < 0:
                break
            out.append(self._check(self.buf[: idx + 1]))
            self.buf = self.buf[idx + 1 :]
        return "".join(out)

    def flush(self) -> str:
        rest, self.buf = self.buf, ""
        return self._check(rest) if rest else ""
