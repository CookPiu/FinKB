...
def run_import(path: Path) -> None:
    with _import_lock:
        ...
        except Exception as e:  # noqa: BLE001 - 文档状态已由节点标记为 failed
            logger.error("导入失败 %s：%s", path.name, e)
...
def list_documents():
    ...
    docs = mongo.get_db().documents.find({"status": {"$ne": "superseded"}}, fields).sort("file_name", 1)
    return {"documents": [{**d, "doc_id": d.pop("_id")} for d in docs]}
